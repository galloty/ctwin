...
private:
	...
	cl_mem _prime_vector = nullptr, _prime_3_4_vector;
	cl_mem _ext_vector = nullptr, _ext_3_4_vector = nullptr;
	cl_mem _factor_vector = nullptr;
	cl_mem _prime_count = nullptr, _prime_3_4_count = nullptr, _factor_count = nullptr;

	cl_kernel _generate_primes = nullptr, _init_factors = nullptr, _generate_factors = nullptr, _clear_primes = nullptr;
	cl_kernel _init_3_4_factors = nullptr, _generate_3_4_factors = nullptr;
...
public:
	void createKernels(const int mode)
	{
		...
		if (mode < 0)
		{
			...
			_setKernelArg(_generate_factors, 1, sizeof(cl_mem), &_prime_vector);
			_setKernelArg(_generate_factors, 2, sizeof(cl_mem), &_ext_vector);
			_setKernelArg(_generate_factors, 3, sizeof(cl_mem), &_kro_vector);
			_setKernelArg(_generate_factors, 4, sizeof(cl_mem), &_factor_count);
			_setKernelArg(_generate_factors, 5, sizeof(cl_mem), &_factor_vector);
			...
			_setKernelArg(_generate_3_4_factors, 3, sizeof(cl_mem), &_kro_vector);
			_setKernelArg(_generate_3_4_factors, 4, sizeof(cl_mem), &_factor_count);
			_setKernelArg(_generate_3_4_factors, 5, sizeof(cl_mem), &_factor_vector);
		}
	}
...
public:
	...
	{
		const cl_ulong i = cl_ulong(index);
		_setKernelArg(_generate_primes, 6, sizeof(cl_ulong), &i);
		_executeKernel(_generate_primes, count);
	}
